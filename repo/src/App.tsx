...
export default function CanvasDefender() {
            ...
            const updated = enemies.map(e => {
              ...
              if (eDist < hitRange) {
                ...
                if (newHealth <= 0) {
                  ...
                  setScore(s => s + 15 + Math.floor(e.maxHealth / 10) + (e.isElite ? 25 : 0));
  ...
  useEffect(() => {
    if (campaignEvents.length > 0) {
      const hasTowerBuff = campaignEvents.some(e => e.effect.type === 'towerBuff' && e.remainingWaves > 0);
  ...
  const handleSelectReward = (reward: Reward) => {
    if (reward.effect.type === 'coreHeal') {
      ...
      if (healAmount >= 999) {
        ...
        setCoreHealth(h => Math.min(h + healAmount, 100));
      }
    }
    ...
    if (shouldTriggerEvent(wave)) {
      ...
      if (evt) {
        setCampaignEvents(prev => [...prev, evt]);
    ...
    const campaignState: CampaignState = {
      ...
      activeEvents: campaignEvents.filter(e => e.remainingWaves > 0),
    ...
    setCampaignEvents(prev =>
      prev.map(updateEventRemaining).filter(e => !isEventExpired(e))
    );
  ...
  const loadSavedGame = () => {
    ...
    for (const reward of build.selectedRewards) {
      setActiveModifiers(prev => applyRewardEffect(reward, prev));
    }
  ...
  const handleGameEnd = (isVictory: boolean) => {
    ...
    const updatedReplay = finalizeReplay(replay, {
      ...
      enemiesKilled,
      buildTags: playerBuild.buildTags,
      selectedRewards: playerBuild.selectedRewards.map(r => ({ name: r.name, rarity: r.rarity })),
    });
    ...
    <div className="min-h-screen bg-amber-50 flex flex-col items-center justify-center p-4"
      ...
      {gameState === 'victory' && currentReplay && (
        <CampaignSummary
          ...
          towerColorCounts={{
            red: towers.filter(t => t.type === 'red').length,
            blue: towers.filter(t => t.type === 'blue').length,
            yellow: towers.filter(t => t.type === 'yellow').length,
          }}
          onRestart={startGame}
