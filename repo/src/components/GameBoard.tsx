...
export const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const PATH_LINES = PATH.reduce<{ x1: number; y1: number; x2: number; y2: number }[]>((lines, pos, i) => {
    if (i < PATH.length - 1) {
      lines.push({
        ...
        >
          <div
            ...
            style={{
              ...
              borderColor: getTowerColorValue(tower.type),
              backgroundColor: getTowerColorValue(tower.type),
            }}
          />

          <div
            className={`w-10 h-10 flex items-center justify-center transition-transform hover:scale-110 shadow-lg ${getTowerStyleClass(tower.style)}`}
            style={{
              background: `linear-gradient(135deg, ${getTowerColorValue(tower.type)}dd, ${getTowerColorValue(tower.type)})`,
              borderRadius:
                tower.style === 'oil'
                  ...
                  ? '50% 50% 50% 50%'
                  : '8px',
              boxShadow: `0 4px 12px ${getTowerColorValue(tower.type)}60, inset 0 0 10px rgba(255,255,255,0.3)`,
              border: tower.style === 'pencil' ? '2px dashed #333' : `3px solid ${getTowerColorValue(tower.type)}`,
            }}
          >
