...
import { TOWER_STYLE_META, getStyleClass, getTowerColor } from '../config/towerConfig';
import { getTowerUpgradeCost } from '../logic/towerSystem';
import type { Enemy, Particle, Projectile, Tower } from '../types/entities';
import type { GameStatus, TowerType } from '../types/game';
...
interface GameBoardProps {
  ...
  canPlaceTowerAt: (x: number, y: number) => boolean;
...
export const GameBoard = ({
  ...
  <div className="flex flex-col items-center">
        ...
        >
          <div
            ...
            style={{
              ...
              borderColor: getTowerColor(tower.type),
          ...
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${getTowerColor(tower.type)}dd, ${getTowerColor(tower.type)})`,
              borderRadius:
                tower.style === 'oil'
                  ? '30% 70% 70% 30% / 30% 30% 70% 70%'
                  : tower.style === 'watercolor'
                    ? '50% 50% 50% 50%'
                    : '8px',
              boxShadow: `0 4px 12px ${getTowerColor(tower.type)}60, inset 0 0 10px rgba(255,255,255,0.3)`,
              border: tower.style === 'pencil' ? '2px dashed #333' : `3px solid ${getTowerColor(tower.type)}`,
            }}
          >
            <span className="text-lg text-white font-bold drop-shadow-lg">
              {TOWER_STYLE_META[tower.style].icon}
              {tower.level}
            </span>
          </div>
          ...
          <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-gradient-to-r from-gray-800 to-gray-700 text-white text-xs px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 shadow-lg border border-gray-600">
            {tower.level < maxTowerLevel ? `⬆️ 升级: ${getTowerUpgradeCost(tower)}精华` : '⭐ 已满级'}
          </div>
        </div>
      ))}
      ...
      {projectiles.map((projectile) => (
        <div
          ...
          style={{
            ...
            border: projectile.type === 'slow' ? '2px dashed white' : 'none',
          }}
        />
      ))}
    ...
    <div className="flex gap-3 mt-3">
      ...
      {!waveInProgress && wave < maxWave && enemies.length === 0 && (
        <button
          onClick={onSkipWave}
